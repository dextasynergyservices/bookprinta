...
import { ProcessingProgressStepper } from "./processing-progress-stepper";
...
): "AI_FORMATTING" | "COUNTING_PAGES" | "RENDERING_PREVIEW" | null {
...
export function ManuscriptPreviewPanel({
  ...
  const [partialPreviewHtml, setPartialPreviewHtml] = useState<string | null>(null);
  const previousProcessingVisibleRef = useRef(false);

  // SSE stream for partial preview URL (first-chunk preview)
  const sse = useProcessingStream(bookId, processing.isActive);

  // Fetch partial preview HTML when SSE sends a first-chunk URL
  useEffect(() => {
    if (!sse.partialPreviewUrl) return;
    let cancelled = false;
    const controller = new AbortController();

    fetch(sse.partialPreviewUrl, { method: "GET", cache: "no-store", signal: controller.signal })
      .then(async (res) => {
        if (!res.ok || cancelled) return;
        const html = await res.text();
        if (!cancelled) setPartialPreviewHtml(sanitizeBookPreviewHtml(html));
      })
      .catch(() => {
        // Non-critical — full preview will replace this
      });
  ...
  }, [sse.partialPreviewUrl]);

  // Clear partial preview when full preview arrives
  useEffect(() => {
    if (previewHtml) setPartialPreviewHtml(null);
  }, [previewHtml]);
  ...
  const hasPendingSettingsChange =
    Boolean(selectedPageSize && committedPageSize && selectedPageSize !== committedPageSize) ||
    Boolean(selectedFontSize && committedFontSize && selectedFontSize !== committedFontSize);
  ...
  return (
    <section className="rounded-2xl border border-[#2A2A2A] bg-[#111111] p-4 md:p-5">
      ...
      <div className="mt-4 space-y-5">
        ...
        {hasPendingSettingsChange && !isSavingSettings ? (
          <p className="font-sans flex items-center gap-2 text-sm text-[#9fbce0]">
            <span className="inline-block h-3 w-3 animate-spin rounded-full border-2 border-[#007eff] border-t-transparent" />
            {tDashboard("book_progress_settings_pending")}
          </p>
        ) : null}

        {showReprocessingState ? (
          <>
            <ProcessingProgressStepper
              bookId={bookId}
              processing={processing}
              wordCount={wordCount}
              hasUploadedManuscript={hasUploadedManuscript}
              isSettingsRerun={isSettingsRerun}
            />
            {partialPreviewHtml && previewPageSize && previewFontSize ? (
              <div className="mt-3">
                <p className="font-sans mb-2 text-xs font-medium text-[#9fbce0]">
                  {tDashboard("book_progress_partial_preview_label")}
                </p>
                <div className="relative aspect-[3/4] max-h-[480px] w-full overflow-hidden rounded-xl border border-[#007eff]/25 bg-white">
                  <iframe
                    title="Partial preview"
                    srcDoc={buildPreviewSrcDoc({
                      html: partialPreviewHtml,
                      pageSize: previewPageSize,
                      fontSize: previewFontSize,
                    })}
        ...
        <div className="overflow-hidden rounded-[28px] border border-[#2A2A2A] bg-[#080808]">
          {!hasUploadedManuscript ? (
            ...
            <div className="flex min-h-[320px] flex-col items-center justify-center gap-3 px-6 py-10 text-center">
              <div className="flex size-14 items-center justify-center rounded-full border border-[#1d344f] bg-[#08111d]">
                <FileText className="size-6 text-[#007eff]" aria-hidden="true" />
              </div>
              <p className="font-sans max-w-lg text-sm text-[#9fbce0]">
                {tDashboard("processing_preview_placeholder")}
              </p>
            </div>
          ) : isLoadingPreview ? (
