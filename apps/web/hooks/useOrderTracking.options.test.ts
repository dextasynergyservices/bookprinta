...
import { orderTrackingQueryKeys, useOrderTracking } from "./useOrderTracking";
...
describe("useOrderTracking query options", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("uses the shared dashboard status polling policy for order tracking", () => {
    renderHook(() =>
      useOrderTracking({ orderId: "cmorder1111111111111111111111", enabled: false })
    );

    const options = useQueryMock.mock.calls[0]?.[0] as QueryOptionsShape;
    expect(options.queryKey).toEqual(
      orderTrackingQueryKeys.detail("cmorder1111111111111111111111")
    );
