...
import type { AdminPaymentSortField, PaymentProvider, PaymentStatus } from "@bookprinta/shared";
import { useSearchParams } from "next/navigation";
import { startTransition, useCallback, useMemo } from "react";
import { usePathname, useRouter } from "@/lib/i18n/navigation";

export const ADMIN_PAYMENTS_LIMIT = 20;
export const DEFAULT_ADMIN_PAYMENT_SORT_BY: AdminPaymentSortField = "createdAt";
export const DEFAULT_ADMIN_PAYMENT_SORT_DIRECTION = "desc" as const;

const ADMIN_PAYMENT_SORT_FIELDS: AdminPaymentSortField[] = [
  "orderReference",
  "customerName",
  "customerEmail",
  "amount",
...
export const ADMIN_PAYMENT_PROVIDER_OPTIONS: PaymentProvider[] = [
  "PAYSTACK",
  "STRIPE",
  "PAYPAL",
  "BANK_TRANSFER",
...
function normalizeSortField(value: SearchParamValue): AdminPaymentSortField {
  const normalized = value?.trim() ?? "";
  return ADMIN_PAYMENT_SORT_FIELDS.includes(normalized as AdminPaymentSortField)
    ? (normalized as AdminPaymentSortField)
    : DEFAULT_ADMIN_PAYMENT_SORT_BY;
}

function normalizeSortDirection(value: SearchParamValue): "asc" | "desc" {
  return value === "asc" ? "asc" : DEFAULT_ADMIN_PAYMENT_SORT_DIRECTION;
}
...
export function humanizeAdminPaymentValue(value: string | null | undefined): string {
...
export function useAdminPaymentsFilters() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const pathname = usePathname();

  const filters = useMemo(() => {
    const status = normalizeEnumParam(searchParams.get("status"), ADMIN_PAYMENT_STATUS_OPTIONS, "");
    const provider = normalizeEnumParam(
  ...
  const setSort = useCallback(
    (sortBy: AdminPaymentSortField, sortDirection: "asc" | "desc") => {
      replaceParams((params) => {
        if (sortBy === DEFAULT_ADMIN_PAYMENT_SORT_BY) {
          params.delete("sortBy");
        } else {
          params.set("sortBy", sortBy);
        }

        if (sortDirection === DEFAULT_ADMIN_PAYMENT_SORT_DIRECTION) {
